...
# kritarunner.exe (optional - only present when pykrita Python plugin was built)
if os.path.isfile(f"{KRITA_INSTALL_DIR}\\bin\\kritarunner.exe"):
    ...
    if os.path.isfile(f"{KRITA_INSTALL_DIR}\\bin\\kritarunner.pdb"):
        shutil.copy(f"{KRITA_INSTALL_DIR}\\bin\\kritarunner.pdb",
                    f"{pkg_root}\\bin\\")
    if os.path.isfile(f"{KRITA_INSTALL_DIR}\\bin\\kritarunner_com.com"):
        shutil.copy(f"{KRITA_INSTALL_DIR}\\bin\\kritarunner_com.com",
                    f"{pkg_root}\\bin\\")
else:
    print("Note: kritarunner.exe not found - skipping (pykrita not built)")
