...
class PackageMSVC(MakeFilePackageBase):
    def __init__(self, **kwargs):
        ...
        config = {
            ...
            "CRAFT_WIN64": "" if CraftCore.compiler.architecture in (CraftCompiler.Architecture.x86_64, CraftCompiler.Architecture.arm64) else "undef",
    ...
    def _globEnv(self):
        ...
        if CraftCore.compiler.isMSVC():
            # The perl win32 Makefile invokes bare `miniperl` (no path) after `cd ..`,
            # relying on cmd executing it from the current directory. Hardened Windows
            # (NoDefaultCurrentDirectoryInPath semantics) disables cwd execution, so we
            # put the perl source root (where miniperl.exe is built) on PATH instead.
