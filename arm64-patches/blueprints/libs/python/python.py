...
if CraftCore.compiler.isMSVC():
    ...
    class Package(MSBuildPackageBase):
        ...
        def configure(self, defines=""):
            _arch = "arm64" if CraftCore.compiler.architecture == CraftCore.compiler.Architecture.arm64 else "x64"
            vars = {"VCPKG_LIBRARY_LINKAGE": "dynamic", "CURRENT_INSTALLED_DIR": CraftCore.standardDirs.craftRoot(), "VCPKG_TARGET_ARCHITECTURE": _arch}
        ...
        def install(self):
            ...
            # CPython's PCbuild emits per-arch output dirs: amd64 (x64), arm64 (ARM64).
            outArch = "arm64" if CraftCore.compiler.architecture == CraftCore.compiler.Architecture.arm64 else "amd64"
            for p in ["python", "pythonw", "venvlauncher", "venvwlauncher"]:
                if not utils.copyFile(self.sourceDir() / f"PCbuild/{outArch}/{p}{debugSuffix}.exe", self.imageDir() / f"bin/{p}{debugSuffix}.exe"):
                    return False
                if self.buildType() == "Debug":
                    if not utils.createShim(self.imageDir() / f"bin/{p}.exe", self.imageDir() / f"bin/{p}{debugSuffix}.exe"):
                        return False
            if not self._globCopy(self.sourceDir() / f"PCbuild/{outArch}/", self.imageDir() / "bin", ["*.dll"]):
                return False
            for p in ["python3", f"python3{verMinor}"]:
                if not utils.copyFile(self.sourceDir() / f"PCbuild/{outArch}/{p}{debugSuffix}.lib", self.imageDir() / f"lib/{p}.lib"):
                    return False
            if not self._globCopy(self.sourceDir() / f"PCbuild/{outArch}/", self.imageDir() / "bin/DLLs", ["*.pyd"]):
                return False
