...
// removed as gcc 4.1 and msvc 2008 are not supported anyways.
  ...
  #elif ((defined  __ARM_NEON) || (defined __ARM_NEON__)) && !(defined EIGEN_ARM64_USE_SVE) && !(defined _MSC_VER)
    // MSVC ARM64: Eigen's NEON backend uses GCC inline asm (__asm__ __volatile__)
    // and NEON-type initializer lists that MSVC does not accept. Fall through to
    // the scalar path on MSVC.
